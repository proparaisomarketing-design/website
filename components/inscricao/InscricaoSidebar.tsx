...
interface InscricaoSidebarProps {
  currentSection: number
  onNavigate: (section: number) => void
}

const steps = [
  { number: 1, title: 'DADOS DA CRIANÇA/JOVEM' },
  { number: 2, title: 'RESPONSÁVEL' },
  { number: 3, title: 'PROJETOS DESEJADOS' },
  { number: 4, title: 'SAÚDE E EMERGÊNCIA' },
  { number: 5, title: 'AUTORIZAÇÕES E ENVIO' },
]

export default function InscricaoSidebar({ currentSection, onNavigate }: InscricaoSidebarProps) {
  ...
  const getStepDotContent = (stepNumber: number, state: string) => {
    if (state === 'completed') {
      return (
        <svg width="39" height="39" viewBox="0 0 39 39" fill="none" xmlns="http://www.w3.org/2000/svg">
          <circle cx="19.5" cy="19.5" r="16.5" fill="#60852D" />
    ...
    if (state === 'active') {
      return (
        <svg width="39" height="39" viewBox="0 0 39 39" fill="none" xmlns="http://www.w3.org/2000/svg">
          <circle cx="19.5" cy="19.5" r="16.5" fill="#3B95D2" stroke="#F4A900" strokeWidth="4" />
    ...
    return (
      <svg width="39" height="39" viewBox="0 0 39 39" fill="none" xmlns="http://www.w3.org/2000/svg">
        <circle cx="19.5" cy="19.5" r="16.5" fill="#1D2130" />
        <text x="19.5" y="19.5" fontFamily="Arial, sans-serif" fontSize="16" fontWeight="bold" fill="white" textAnchor="middle" dominantBaseline="central">{stepNumber}</text>
      </svg>
    )
  }
            ...
            <div className="steps" onClick={() => onNavigate(0)}>
              ...
              <div className="step-texts">
                <div className={`step-title ${currentSection === 0 ? 'active' : 'future'}`}>
                  <h2 style={{ marginBottom: '15px' }}>PRÉ-INSCRIÇÃO</h2>
                </div>
              </div>
            </div>
          </div>
                ...
                {!isLast && (
                  <div className={`process-bar ${state === 'active' || state === 'completed' ? 'active' : ''}`}>
                    <svg width="3" height="20" viewBox="0 0 3 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <rect width="3" height="20" fill={state === 'active' || state === 'completed' ? '#60852D' : '#CFD6DC'} />
                    </svg>
                  </div>
                )}
      ...
      <div className="bottom-wrapper">
        <div className="help-content">
          <div className="help-texts">
            <div className="help-title">Dúvidas?</div>
            <div className="help-paragraph">Fale com a Associação Pró-Paraíso no WhatsApp</div>
          </div>
          <a
            href="https://api.whatsapp.com/send/?phone=5511987103256&text=Olá,%20estou%20fazendo%20a%20pré-inscrição%20e%20preciso%20de%20ajuda"
            target="_blank"
